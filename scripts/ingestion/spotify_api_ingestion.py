...
class SpotifyAPIClient:
    ...
    def __init__(self, client_id: str = None, client_secret: str = None):
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError("Spotify client credentials not found")
    ...
    def get_recently_played(self, limit: int = 50, before: str = None, after: str = None) -> Dict[str, Any]:
...
class SpotifyDataIngestion:
    """Handles ingestion of Spotify data into DuckDB."""
    ...
    def fetch_recently_played(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Fetch recently played tracks from Spotify API."""
        try:
            logger.info(f"Fetching {limit} recently played tracks from Spotify")

            response = self.spotify_client.get_recently_played(limit=limit)
    ...
    def enrich_track_data(self, tracks_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _enrich_single_track(self, item: Dict[str, Any],
                           track_details: Dict[str, Any],
                           artist_details: Dict[str, Any],
                           album_details: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def run_ingestion(self, limit: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            raw_data = self.fetch_recently_played(limit=limit)
            if not raw_data:
                return {"status": "no_data", "message": "No data retrieved from Spotify"}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Spotify API Data Ingestion')
    parser.add_argument('--limit', type=int, default=50,
                       help='Number of tracks to fetch (max 50)')
    parser.add_argument('--duckdb-path', type=str,
                       help='Path to DuckDB database file')

    args = parser.parse_args()

    ingestor = SpotifyDataIngestion(duckdb_path=args.duckdb_path)
    result = ingestor.run_ingestion(limit=args.limit)
