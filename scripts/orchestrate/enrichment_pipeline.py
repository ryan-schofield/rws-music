...
class EnrichmentPipeline:
    ...
    def validate_prerequisites(self) -> Dict[str, Any]:
        ...
        if validation_results["tracks_played_exists"]:
            ...
            logger.info(f"tracks_played table contains {tracks_info.get('record_count', 0)} records")
        else:
            logger.warning("tracks_played table not found - enrichment may have limited data")
    ...
    def run_full_pipeline(self, skip_on_error: bool = False, 
    ...
    def _finalize_pipeline(self, results: Dict[str, Any], status: str) -> Dict[str, Any]:
        ...
        logger.info(f"Completed processors: {', '.join(self.pipeline_state['processors_completed'])}")
        
        if self.pipeline_state["processors_failed"]:
            logger.warning(f"Failed processors: {', '.join(self.pipeline_state['processors_failed'])}")
    ...
    def _get_table_status(self) -> Dict[str, Any]:
        ...
        tables = [
            "tracks_played", "spotify_artists", "spotify_albums", "spotify_artist_genre",
            "mbz_artist_info", "mbz_area_hierarchy", "cities_with_lat_long"
        ]
