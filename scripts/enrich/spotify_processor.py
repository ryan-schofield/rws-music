...
class SpotifyProcessor:
    ...
    def __init__(self, data_writer: ParquetDataWriter = None, 
    ...
    def enrich_artists(self) -> Dict[str, Any]:
        ...
        logger.info(f"Found {len(missing_artists_df)} artists needing Spotify enrichment")
        ...
        try:
            artist_data = self.spotify_client.get_artists_batch(artist_ids, batch_size=50)
            ...
            expected_columns = ["artist_id", "artist_name", "artist_mbid", "artist_popularity"]
    ...
    def _create_artist_genre_table(self, artist_df: pl.DataFrame) -> Optional[Dict[str, Any]]:
        """
    ...
    def _create_album_genre_table(self, album_df: pl.DataFrame) -> Optional[Dict[str, Any]]:
        """
    ...
    def update_artist_mbids(self) -> Dict[str, Any]:
        ...
        try:
            ...
            updated_artists = artists_needing_mbid.join(
                mbz_artist_df.select(["spotify_id", "id"]).rename({"id": "artist_mbid_new"}),
                left_on="artist_id",
                right_on="spotify_id",
                how="left"
            )
            ...
            updated_artists = updated_artists.with_columns(
                pl.coalesce([pl.col("artist_mbid_new"), pl.col("artist_mbid")]).alias("artist_mbid")
