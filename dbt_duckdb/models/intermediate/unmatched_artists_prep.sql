WITH artist_delimited_cte
AS (
    SELECT artist AS artist_name
        , artist_mbid
        , COUNT(*) AS count_played
        , REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(artist, ',', '|'), 'feat.', '|'), '[', '|'), '(', '|'), ' & ', '|') AS artist_delimited
...
AS ( 
    SELECT a.artist_name
        , a.artist_mbid
        , s.value AS potential_match
        , count_played
    FROM artist_delimited_cte a
    CROSS APPLY STRING_SPLIT(a.artist_delimited, '|', 1) s
    WHERE s.ordinal = 1
 )
