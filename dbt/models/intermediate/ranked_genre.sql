...
SELECT
    ...
    , CASE WHEN row_num_tracks = total_rows THEN {{ groups }} ELSE (row_num_tracks / count_per_group) + 1 END
        AS tracks_grouping
    , total_tracks_played_in_genre
    , row_num_ms AS rank_by_time_played
    , CASE WHEN row_num_ms = total_rows THEN {{ groups }} ELSE (row_num_ms / count_per_group) + 1 END AS time_grouping
